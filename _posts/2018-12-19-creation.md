...
그런데 원점을 중심으로 x+ 방향과 y+ 방향의 색상은 매우 뚜렷해서 거의 1에 가까운 값이 나오는 것 같습니다. 이를 확인하기 위해 13행의 주석을 해제해보면, 실제로 `p/l` 의 `x` 는 $$x \ge 0.0$$ 이고 $$y \simeq 0.0$$ 인 구간에서 `1.0` 이상의 큰 값을 갖습니다. 저는 처음에는 이 부분이 잘 이해가 안됐습니다만, 사실 `l=length(p)` 이기 때문에 `l`은 `p.y=0` 일 때 `p.x` 와 같은 값이 됩니다. ($$l = \sqrt{p.x^2+p.y^2}$$) `p.y` 가 아주 작은 값이 되는 x+ 축 주변에서는 여전히 1에 가까운 값을 갖습니다.
