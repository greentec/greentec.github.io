function A3CAgent_Worker(_env, _x, _y, _canvas, shared_agent) {
    ...
    this.train_model = async function(state, action, reward, next_state, done) {
        ...
        const value = this.critic.predict(input);
        const next_value = this.critic.predict(next_input);

        const value_v = value.dataSync();
        const next_value_v = next_value.dataSync();
